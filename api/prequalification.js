...
export default async function handler(req, res) {
  ...
  const key = process.env['CLÉ_API_BREVO'] || process.env.BREVO_API_KEY
  if (!key) return res.status(500).json({ error: 'Clé Brevo manquante' })
  ...
  // Expéditeur explicite — obligatoire pour les emails transactionnels Brevo
  const sender = { name: 'Vivre à Majorque', email: AMELY_EMAIL }

  try {
    /* ── 1. Notifier Amely ── */
    const r1 = await fetch(`${BREVO_API}/smtp/email`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        sender,
        ...
        htmlContent: `
          ...
          <table style="border-collapse:collapse;width:100%;font-family:sans-serif;font-size:14px">
            <tr><td style="padding:8px;background:#f5f0e8;font-weight:700;width:140px">Offre</td><td style="padding:8px;border-bottom:1px solid #ddd"><strong style="color:#C76E4E">${offre}</strong></td></tr>
            ...
            <tr><td style="padding:8px;background:#f5f0e8;font-weight:700">Budget OK</td><td style="padding:8px">${budget_conscience ? '✅ Oui' : '❌ Non'}</td></tr>
          </table>
          <p style="margin-top:16px;color:#666;font-size:13px">→ Répondre sous 24h et envoyer le lien de paiement.</p>
        `,
      }),
    })
    const d1 = await r1.json().catch(() => ({}))
    if (!r1.ok) {
      console.error('Brevo email 1 error:', r1.status, JSON.stringify(d1))
      return res.status(502).json({ error: 'Erreur envoi email Amely', detail: d1 })
    }

    /* ── 2. Confirmation au prospect ── */
    const r2 = await fetch(`${BREVO_API}/smtp/email`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        sender,
        ...
        htmlContent: `
          <div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1C1410;line-height:1.7">
            ...
            <p>En attendant, n'hésitez pas à me répondre directement sur cet email.</p>
    ...
    if (!r2.ok) {
      console.error('Brevo email 2 error:', r2.status, JSON.stringify(d2))
      // On continue même si la confirmation prospect échoue
    }
