  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', paddingBottom: step === 3 ? 120 : 80 }}>
      ...
      <div style={{ background: 'var(--foret)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        {step === 3 && (
          <span style={{ marginLeft: 'auto', fontSize: 16, fontWeight: 700, color: 'var(--gold)', fontFamily: 'var(--font-corps)' }}>{fmt(total)}</span>
        )}
      </div>
        ...
        {step === 1 && (
          <div>
            <div style={{ background: '#fff', borderRadius: 14, padding: '16px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
              ...
              {/* Adultes */}
              <p style={{ margin: '0 0 8px', fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-corps)', color: 'var(--noir)' }}>Combien d'adultes ?</p>
              <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
                {[{ n: 1, label: '🧑 Seul(e)' }, { n: 2, label: '👫 En couple' }].map(o => (
                  <button key={o.n} onClick={() => setAdultes(o.n)} style={{
                    ...
                    fontFamily: 'var(--font-corps)', fontSize: 14, cursor: 'pointer', fontWeight: adultes === o.n ? 700 : 400,
                  }}>{o.label}</button>
                ))}
              </div>

              {/* Enfants */}
              <p style={{ margin: '0 0 6px', fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-corps)', color: 'var(--noir)' }}>Enfants à charge</p>
              <p style={{ margin: '0 0 10px', fontSize: 13, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)' }}>Cliquez sur une tranche d'âge pour ajouter un enfant</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                {TRANCHES.map(t => (
                  <button key={t} onClick={() => addEnfant(t)} style={{
                    ...
                    <span style={{ lineHeight: 1.25, textAlign: 'left', fontSize: 13, color: 'var(--noir)' }}>{TRANCHE_LABELS[t]}</span>
                  </button>
                ))}
              </div>
              {enfants.length > 0 ? (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                  {enfants.map((t, i) => (
                    <button key={i} onClick={() => removeEnfant(i)} style={{
                      ...
                      <span style={{ fontSize: 13, color: '#fff', fontFamily: 'var(--font-corps)' }}>{TRANCHE_LABELS[t]}</span>
                      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)' }}>✕</span>
                    </button>
                  ))}
                </div>
              ) : (
                <p style={{ margin: 0, fontSize: 13, color: 'var(--texte-sec)', fontStyle: 'italic', fontFamily: 'var(--font-corps)', textAlign: 'center', padding: '6px 0' }}>
            ...
            {hasScolaires && (
              <div style={{ background: '#fff', borderRadius: 14, padding: '16px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                ...
                <p style={{ margin: '0 0 12px', fontSize: 13, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {ECOLE_OPTIONS.map(opt => (
                    <button key={opt.id} onClick={() => setEcoleType(opt.id)} style={{
                      ...
                      <span style={{ fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-corps)', color: 'var(--noir)' }}>{opt.label}</span>
                      <span style={{ fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', marginTop: 2 }}>{opt.sub}</span>
                    </button>
                  ))}
                </div>
                <p style={{ margin: '10px 0 0', fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', lineHeight: 1.50 }}>
            ...
            <button onClick={() => setStep(2)} style={{
              ...
              background: 'var(--foret)', color: '#fff', fontSize: 16, fontWeight: 700,
        ...
        {step === 2 && (
          <div>
            <div style={{ background: '#fff', borderRadius: 14, padding: '16px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
              ...
              <p style={{ margin: '0 0 14px', fontSize: 13, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)' }}>
                ...
                {MODES.map(m => {
                  ...
                  return (
                    <button key={m.id} onClick={() => setMode(m.id)} style={{
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontFamily: 'var(--font-corps)', color: 'var(--noir)', fontSize: 16 }}>{m.label}</div>
                        <div style={{ fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', marginTop: 2 }}>{m.desc}</div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--foret)', fontFamily: 'var(--font-corps)' }}>{fmt(previewTotal)}</div>
                        <div style={{ fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)' }}>/mois</div>
                      </div>
                    </button>
                  )
                })}
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ background: 'var(--lin)', borderRadius: 12, padding: '10px 14px', marginBottom: 16, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
              <span style={{ fontSize: 13, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)' }}>
                {adultes === 1 ? '🧑 Seul(e)' : '👫 En couple'}
                {enfants.length > 0 && ` + ${enfants.length} enfant${enfants.length > 1 ? 's' : ''}`}
              </span>
              <span style={{ fontSize: 13, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)' }}>
              ...
              <button onClick={() => setStep(1)} style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--vert)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-corps)', textDecoration: 'underline' }}>Modifier</button>
            </div>
            ...
            <div style={{ background: '#fff', borderRadius: 14, padding: '16px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                ...
                {visibleCats.filter(c => budget[c.id] > 0).sort((a, b) => budget[b.id] - budget[a.id]).map(c => (
                  <div key={c.id}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                      <span style={{ fontSize: 13, fontFamily: 'var(--font-corps)', color: 'var(--noir)' }}>{c.emoji} {c.label}</span>
                      <span style={{ fontSize: 13, fontWeight: 600, fontFamily: 'var(--font-corps)', color: 'var(--noir)' }}>{fmt(budget[c.id])}</span>
                    </div>
              ...
              <p style={{ margin: '12px 0 0', fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 14, padding: '16px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                ...
                <button onClick={resetOverrides} style={{ fontSize: 12, color: 'var(--vert)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-corps)', textDecoration: 'underline' }}>Réinitialiser</button>
              </div>

              {visibleCats.map(cat => (
                <div key={cat.id} style={{ marginBottom: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                    <label style={{ fontSize: 14, fontFamily: 'var(--font-corps)', color: 'var(--noir)', fontWeight: 500 }}>{cat.emoji} {cat.label}</label>
                    {overrides[cat.id] !== undefined && <span style={{ fontSize: 12, color: 'var(--terra)', fontFamily: 'var(--font-corps)' }}>✎ modifié</span>}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <input
                      ...
                      style={{
                        ...
                        fontSize: 16, fontFamily: 'var(--font-corps)', fontWeight: 600, textAlign: 'right',
                        background: overrides[cat.id] !== undefined ? '#fff8f0' : 'var(--lin)', color: 'var(--noir)',
                      }}
                    />
                    <span style={{ fontSize: 13, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', minWidth: 14 }}>€</span>
                  </div>
                  {cat.id === 'loyer' && (
                    <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', lineHeight: 1.40 }}>
            ...
            <div style={{ background: 'var(--foret)', borderRadius: 16, padding: '20px 16px', marginBottom: 20 }}>
                ...
                {['Logement', 'Alimentation', 'Transport', 'Santé', 'Famille', 'Loisirs', 'Perso'].map(group => {
                  ...
                  return (
                    <div key={group} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 14, color: 'rgba(255,255,255,0.8)', fontFamily: 'var(--font-corps)' }}>
                        {cats[0]?.emoji} {group}
                      </span>
                      <span style={{ fontSize: 16, fontWeight: 600, color: '#fff', fontFamily: 'var(--font-corps)' }}>{fmt(groupTotal)}</span>
                    </div>
                  )
                })}
              </div>
              ...
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.25)', paddingTop: 14 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', fontFamily: 'var(--font-corps)' }}>Soit par an</span>
                  <span style={{ fontSize: 16, fontWeight: 600, color: 'rgba(255,255,255,0.9)', fontFamily: 'var(--font-corps)' }}>{fmt(total * 12)}</span>
                </div>
              </div>

              <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 10, padding: '12px', marginTop: 14 }}>
                <p style={{ margin: '0 0 6px', fontSize: 13, color: 'rgba(255,255,255,0.8)', fontFamily: 'var(--font-corps)', fontWeight: 600 }}>💡 Revenu net nécessaire</p>
                <p style={{ margin: 0, fontSize: 13, color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-corps)', lineHeight: 1.50 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: '14px 16px', marginBottom: 20, border: '1px solid var(--gris)' }}>
              <p style={{ margin: 0, fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', lineHeight: 1.65 }}>
      ...
      {step === 3 && (
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', fontFamily: 'var(--font-corps)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Total mensuel</div>
            <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--gold)', fontFamily: 'var(--font-corps)', lineHeight: 1.25 }}>{fmt(total)}</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', fontFamily: 'var(--font-corps)' }}>Par an</div>
            <div style={{ fontSize: 16, fontWeight: 600, color: 'rgba(255,255,255,0.9)', fontFamily: 'var(--font-corps)' }}>{fmt(total * 12)}</div>
          </div>
        </div>
      )}
