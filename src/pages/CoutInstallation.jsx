...
function SourceBadge({ src }) {
  return (
    <a href={src.url} target="_blank" rel="noreferrer" style={{
      ...
      fontSize: 12, color: 'var(--texte-sec)', background: 'var(--gris)',
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', overflow: 'hidden', marginBottom: 16 }}>
      ...
      >
        ...
        <span style={{ color: 'var(--texte-sec)', fontSize: 14, transform: open ? 'rotate(90deg)' : 'none', transition: 'transform 0.2s' }}>›</span>
      </button>
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4, marginBottom: 5 }}>
        <label style={{ fontSize: 14, fontWeight: 500, color: 'var(--noir)', fontFamily: 'var(--font-corps)' }}>{label}</label>
        {source && <SourceBadge src={source} />}
      </div>
      {note && <p style={{ fontSize: 12, color: 'var(--texte-sec)', margin: '0 0 5px', fontFamily: 'var(--font-corps)', lineHeight: 1.40 }}>{note}</p>}
      {children}
    </div>
  )
}
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <input
        ...
        style={{
          ...
          fontSize: 16, fontFamily: 'var(--font-corps)', outline: 'none',
          background: 'var(--lin)', color: 'var(--noir)', boxSizing: 'border-box',
        }}
      />
      {suffix && <span style={{ fontSize: 14, color: 'var(--texte-sec)', whiteSpace: 'nowrap' }}>{suffix}</span>}
    </div>
  )
}
    ...
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4 }}>
          <span style={{ fontSize: 14, color: 'var(--noir)', fontFamily: 'var(--font-corps)' }}>{label}</span>
          {source && <SourceBadge src={source} />}
        </div>
        {sublabel && <div style={{ fontSize: 12, color: 'var(--texte-sec)', marginTop: 2, fontFamily: 'var(--font-corps)' }}>{sublabel}</div>}
      </div>
      <div style={{
        fontWeight: 600, fontSize: 16, fontFamily: 'var(--font-corps)',
...
function Toggle({ checked, onChange, label }) {
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '4px 0' }}>
      ...
      <span style={{ fontSize: 14, fontFamily: 'var(--font-corps)', color: 'var(--noir)' }}>{label}</span>
    </label>
  )
}
...
function SubTotal({ label, amount, recoverable }) {
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--foret)', fontFamily: 'var(--font-corps)' }}>{label}</div>
        {recoverable > 0 && <div style={{ fontSize: 12, color: '#2e7d32', fontFamily: 'var(--font-corps)' }}>dont {fmt(recoverable)} récupérables (dépôts)</div>}
      </div>
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', paddingBottom: 100 }}>
      ...
      <div style={{ background: 'var(--foret)', padding: '20px 16px 16px', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 13, color: 'rgba(255,255,255,0.75)', fontFamily: 'var(--font-corps)' }}>Données sources officielles uniquement</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '16px 16px 0' }}>
              ...
              {[0, 1, 2].map(m => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 14, fontWeight: garantieAdd === m ? 600 : 400,
              ...
              {[0, 1, 2, 3].map(m => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 14, fontWeight: chevauchement === m ? 600 : 400,
              ...
              {['UE', 'Non-UE'].map(n => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 14, fontWeight: nationalite === n ? 600 : 400,
        ...
        <SectionCard title="Véhicule immatriculé en France" emoji="🚗" defaultOpen={false}>
          ...
          {withVehicule && (
            <div style={{ marginTop: 14 }}>
              <p style={{ fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', lineHeight: 1.50, marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8 }}>
                  {[
                    ...
                    <label key={t.val} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '6px 10px', borderRadius: 8, background: co2Tranche === t.val ? 'var(--lin)' : 'transparent', border: co2Tranche === t.val ? '1px solid #ddd' : '1px solid transparent' }}>
                      ...
                      <span style={{ fontSize: 13, fontFamily: 'var(--font-corps)', color: t.color }}>{t.label}</span>
                    </label>
                  ))}
                </div>
              </InputRow>
          ...
          {withAutonoma && (
            <div style={{ marginTop: 14 }}>
              <div style={{ background: '#e8f5e9', borderRadius: 8, padding: '10px 12px', marginBottom: 12 }}>
                <p style={{ margin: 0, fontSize: 13, color: '#2e7d32', fontFamily: 'var(--font-corps)', lineHeight: 1.50 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {[6, 12].map(m => (
                    <button
                      ...
                      style={{
                        flex: 1, padding: '10px 0', borderRadius: 8, cursor: 'pointer', fontFamily: 'var(--font-corps)',
                        fontSize: 14, fontWeight: moisTarifaPlana === m ? 600 : 400,
        ...
        <SectionCard title="Trésorerie de sécurité" emoji="🛡️">
          ...
          <p style={{ fontSize: 13, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', lineHeight: 1.50, marginBottom: 12 }}>
          ...
          <div style={{ marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 14, fontFamily: 'var(--font-corps)', fontWeight: 500 }}>Nombre de mois : <strong>{moisReserve}</strong></span>
              <span style={{ fontSize: 14, fontFamily: 'var(--font-corps)', color: 'var(--texte-sec)' }}>Charges : {fmt(calculs.chargesMensuelles)}/mois</span>
            </div>
            <input
              type="range"
              min={1}
            ...
            />
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', marginTop: 2 }}>
        ...
        <div style={{ background: 'var(--foret)', borderRadius: 16, padding: '20px 16px', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 14, color: row.highlight ? 'rgba(255,255,255,0.6)' : 'rgba(255,255,255,0.85)', fontFamily: 'var(--font-corps)' }}>
                  {row.highlight ? '⊕ ' : '·  '}{row.label}
                </span>
                <span style={{ fontSize: 16, fontWeight: 600, color: '#fff', fontFamily: 'var(--font-corps)' }}>{fmt(row.val)}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.25)', paddingTop: 14, marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <span style={{ fontSize: 16, color: 'rgba(255,255,255,0.85)', fontFamily: 'var(--font-corps)' }}>Avant trésorerie</span>
          ...
          {calculs.recoverables > 0 && (
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 12px', marginBottom: 12 }}>
              <p style={{ margin: 0, fontSize: 13, color: 'rgba(255,255,255,0.85)', fontFamily: 'var(--font-corps)' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 12px' }}>
            <p style={{ margin: 0, fontSize: 12, color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-corps)', lineHeight: 1.50 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: '14px 16px', marginBottom: 20, border: '1px solid var(--gris)' }}>
          <h3 style={{ margin: '0 0 10px', fontSize: 14, fontFamily: 'var(--font-titre)', color: 'var(--foret)' }}>Sources officielles utilisées</h3>
          {Object.values(SOURCES).map(s => (
            <div key={s.label} style={{ marginBottom: 4 }}>
              <a href={s.url} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: 'var(--vert)', fontFamily: 'var(--font-corps)', textDecoration: 'none' }}>
                ↗ {s.label}
              </a>
            </div>
          ))}
          <p style={{ margin: '10px 0 0', fontSize: 12, color: 'var(--texte-sec)', fontFamily: 'var(--font-corps)', lineHeight: 1.40 }}>
